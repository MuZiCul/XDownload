...
- **下载历史** — 封面 / 作者 / 时长 / 播放 / 点赞 / 下载时间；即时搜索过滤；时间分级徽标（最近5分钟 → 十年前）；删除时确认是否同时删除磁盘文件；支持重新下载 / 打开位置 / 清空；**同一条视频只保留一条最新记录**——重新下载会替换掉之前那条失败 / 成功记录，不再两条并存
...
- **更新提醒** — 启动时检查 XDownload / yt-dlp / ffmpeg 新版本；ffmpeg 按**内容指纹**比对，刚更新过的版本不会反复提示（点更新前还会再校验一次，不会白下约 195MB）
